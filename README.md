...
- **Debug Tools:** Built-in debug panel for testing (toggle with hotkey)
- **Avatar Customization:** Custom player avatars (requires `avatar.bundle` file)
...
#### Mod Installation
1. Download the latest release of `SailwindCoop.dll`
2. Place `SailwindCoop.dll` in: `Sailwind/BepInEx/plugins/SailwindCoop/`
3. (Optional) Add `avatar.bundle` file to the same folder for custom avatars
4. Launch the game
...
3. Press **F9** to start hosting
4. Share your IP address with friends (see "Finding Your IP" below)
5. Wait for friends to connect

#### Joining a Game
1. Launch Sailwind
2. **Important:** Do NOT load a save game (stay at main menu)
3. Press **F10** to connect to the host
   - Default IP is `127.0.0.1` (localhost)
   - To change: Edit `BepInEx/config/com.sailwind.coop.cfg`
4. Once connected, the host can invite you to their game

#### Disconnecting
- Press **F11** to disconnect from a server
...
- Ensure `avatar.bundle` exists (if using custom avatars)
...
- This mod is in early development (v0.1.0). Expect bugs!
- Only works with players who have the mod installed
- Save games are not synced between players
...
- **Инструменты отладки:** Встроенная панель для тестирования
- **Кастомизация аватаров:** Свои модели игроков (требуется файл `avatar.bundle`)
...
#### Установка мода
1. Скачайте последнюю версию `SailwindCoop.dll`
2. Поместите `SailwindCoop.dll` в: `Sailwind/BepInEx/plugins/SailwindCoop/`
3. (Опционально) Добавьте файл `avatar.bundle` в ту же папку для кастомных аватаров
4. Запустите игру
...
3. Нажмите **F9** чтобы начать хостинг
4. Сообщите друзьям свой IP адрес (см. "Как узнать свой IP" ниже)
5. Ждите подключения друзей

#### Подключение к игре
1. Запустите Sailwind
2. **Важно:** НЕ загружайте сохранение (останьтесь в главном меню)
3. Нажмите **F10** чтобы подключиться к хосту
   - IP по умолчанию: `127.0.0.1` (локальный)
   - Чтобы изменить: отредактируйте `BepInEx/config/com.sailwind.coop.cfg`
4. После подключения хост может пригласить вас в свою игру

#### Отключение
- Нажмите **F11** чтобы отключиться от сервера

#### Оверлей с информацией
- Нажмите **F8** чтобы включить/выключить оверлей с диагностикой сети
...
- Убедитесь, что файл `avatar.bundle` существует (если используете кастомные аватары)
...
- Мод в ранней разработке (v0.1.0). Возможны баги!
- Работает только с игроками, у которых установлен мод
- Сохранения игры не синхронизируются между игроками
