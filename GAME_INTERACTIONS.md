...
## Плавучесть И Волны (Ocean/Crest)

### `Buoyancy`

Компонент на корпусе лодки, толкает её `Rigidbody` силами в `Update`/`FixedUpdate` (`useFixedUpdate`).

Поведение:

- на `SlicesX×SlicesZ` сэмплах днища каждый кадр считает высоту воды через
  `Ocean.Singleton.GetWaterHeightAtLocation[2]` и толкает `rrigidbody.AddForceAtPosition` пропорционально
  погружению точки; плюс демпфирование по вертикальной скорости точки, chop/wind силы (`ChoppynessAffectsPosition`,
  `WindAffectsPosition`), интерполяция силы по последним `interpolation` кадрам.
- Работает **исключительно через `Rigidbody.AddForceAtPosition`**. Если `Rigidbody.isKinematic == true`,
  Unity молча игнорирует силы — `Buoyancy.update()` продолжает выполняться (тратит CPU впустую), но
  вообще не двигает объект.

Вывод для коопа:

- **`BoatSync` уже ставит `Rigidbody.isKinematic = true` у ведомой лодки клиента** → `Buoyancy` там
  полностью недействующий. Высота/крен клиентской лодки — ЦЕЛИКОМ то, что прислал хост в `BoatState`
  (`RealPos.y` + `Rot`), а не реакция на локальную волну клиента.
- На ХОСТЕ `Buoyancy` работает как обычно (не kinematic) — его `Y` корректен относительно ЕГО волн.
  Именно эта `Y`-координата уходит в сеть как истина.

### `Ocean` / Crest (`OceanHeight`, `OceanWavesUpdater`, `OceanUpdaterCrest`)

- Использует сторонний ассет **Crest** (`Crest.SampleHeightHelper`) для процедурных волн. Форма волны в
  `Ocean` считается через `calcComplex(Time.time, 0, height)` — **от `Time.time`**, то есть от реального
  времени работы конкретного Unity-процесса с момента его запуска, а не от игрового `Sun.globalTime`/тика
  хоста.
- Ни `Ocean`, ни Crest-компоненты не входят в `EnvironmentSync` (который синкает только
  ветер/время-суток/луну). Фаза/форма волн расходится между хостом и клиентом с точностью до разницы
  во времени запуска их Unity-процессов и НИКОГДА не сходится сама.

Вывод для коопа:

- Y-координата лодки, присланная хостом, «правильная» относительно волн ХОСТА, но может не совпадать
  с видимой поверхностью воды на клиенте (другая фаза Crest) → лодка клиента визуально то утоплена, то
  приподнята над своей же водой, особенно заметно сразу после коннекта.
- Полноценное исправление требует либо (а) синхронизировать волновой seed/время Crest с хостом (не
  исследовано — сторонняя библиотека, не входит в `Assembly-CSharp`), либо (б) не слать `Y` лодки
  напрямую, а пересчитывать высоту на клиенте локально из `Ocean.GetWaterHeightAtLocation2` в той же
  точке X/Z (сложность: нужна «осадка» лодки, которую сейчас выводит физика `Buoyancy`, а не константа).
  Пока не реализовано ни то, ни другое — известное ограничение, см. `СОСТОЯНИЕ.md` §5.
